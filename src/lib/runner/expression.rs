...
impl Evaluable for Literal {
    fn evaluate(&self, env: &mut Environment) -> Result<Value, String> {
        match self {
            ...
            Literal::Primary(primary) => Ok(Value::Literal(primary.clone())),
        }
    }
}
...
impl Evaluable for Unary {
    fn evaluate(&self, env: &mut Environment) -> Result<Value, String> {
        ...
        let result = match (&self.op, &value) {
            (Not, Value::Literal(Boolean(val))) => Boolean(!val),
            (Not, Value::Literal(Nil)) => Boolean(true),
            (Not, _) => Boolean(false),
            (Minus, Value::Literal(Number(number))) => Number(-number),
